/*****************************************************************************************************************
 *  Copyright Steve Berryman (steveberryman)/ David Lomas (codersaur)
 *
 *  Name: Graphite Logger
 *
 *  Date: 2017-04-03
 *
 *  Version: 1.11
 *
 *  Source: https://github.com/steveberryman/SmartThings/tree/master/smartapps/influxdb-logger
 *
...
definition(
    name: "Graphite Logger",
    namespace: "steveberryman",
    author: "Steve Berryman (steveberryman)",
    description: "Log SmartThings device states to Graphite",
    category: "My Apps",
...
def handleEvent(evt) {
    ...
    def metric = "${locationName}.${hubName}.${groupName}.${deviceName}.${measurement}"
    def val = 0 
    ...
    if ('acceleration' == evt.name) { // acceleration: Calculate a binary value (active = 1, inactive = 0)
        ...
        val = "${valueBinary}"
        data = makeJsonMetric(metric, val)
    }
        ...
        data = makeJsonMetric(metric, "${valueX}")
        data += ","
        data += makeJsonMetric(metric, "${valueY}")
        data += ","
        data += makeJsonMetric(metric, "${valueZ}")
    }
    ...
    if (prefLogHubProperties) {
       	location.hubs.each { h ->
        	try {
                ...
                def batteryInUse = ("false" == h.hub.getDataValue("batteryInUse")) ? "0" : "1"
                def hubUptime = h.hub.getDataValue("uptime")
                def zigbeePowerLevel = h.hub.getDataValue("zigbeePowerLevel")
                ...
                def base = "${locationName}.${hubName}"
                def data = "["
            //  data += makeJsonMetric("${base}.status",hubStatus) + ","
                data += makeJsonMetric("${base}.batteryInUse",batteryInUse) + ","
                data += makeJsonMetric("${base}.uptime",hubUptime) + ","
                data += makeJsonMetric("${base}.zigbeePowerLevel",zigbeePowerLevel) + ","
                data += makeJsonMetric("${base}.zwavePowerLevel",zwavePowerLevel) + "]"
...
def makeJsonMetric(metric, value) {
 ...
 **/
def postToGraphite(metric, value) {
    logger("postToGraphite(): Posting data to Graphite: Host: ${state.databaseHost}, Port: ${state.databasePort}, Database: ${state.databaseName}, Data: [${data}]","debug")
    ...
    try {
        ...
        def hubAction = new physicalgraph.device.HubAction(
            ...
            null,
            [ callback: handleGraphiteResponse ]
        )
		
        sendHubCommand(hubAction)
    }
    catch (Exception e) {
		logger("postToGraphite(): Exception ${e} on ${hubAction}","error")
    }
 ...
 **/
def handleGraphiteResponse(physicalgraph.device.HubResponse hubResponse) {
    if(hubResponse.status >= 400) {
		logger("postToGraphite(): Something went wrong! Response from Graphite Backstop: Headers: ${hubResponse.headers}, Body: ${hubResponse.body}","error")
    }
}
...
private escapeStringForInfluxDB(str) {
    if (str) {
        str = str.replaceAll(" ", "_") // Escape spaces.
...
private getGroupName(id) {
    ...
    else if (id == '40de94a7-59b2-4996-a0cb-2ceafa5dafa1') {return 'living-room'}
    else if (id == '4bee5f1f-fa2f-4e4b-abf0-03c94587a10a') {return 'upstairs-hall'}
    else if (id == 'bcef32eb-0a65-4a2f-a61b-959637e8c712') {return 'downstairs-hall'}
    else if (id == '8cb00422-b118-44a2-a498-99122c73da3a') {return 'steves-bedroom'}
    else if (id == 'dd4fbd25-e740-4d6a-b408-eb0ca6d9446c') {return 'phils-bedroom'}
    else {return 'Unknown'}    
}